...
inline size_t get_bbox_dimensions(SEXP val) {
  typedef size_t fn_ptr(SEXP);
  static fn_ptr *fn = (fn_ptr*) R_GetCCallable("euclid", "get_bbox_dimensions");
  return fn(val);
}
inline size_t get_transform_dimensions(SEXP val) {
  typedef size_t fn_ptr(SEXP);
