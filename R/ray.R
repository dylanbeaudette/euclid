...
ray <- function(..., default_dim = 2) {
  ...
  if (length(points) == 2) {
    ...
    abort("Can't construct a {.cls euclid_ray} vector from the given input")
  }
}
#' @rdname ray
#' @export
...
as_ray.default <- function(x) {
  cli_abort("Can't convert the input to a {.cls euclid_ray} vector")
}
#' @export
...
geometry_op_minus.euclid_ray <- function(e1, e2) {
  if (!missing(e2)) {
    cli_abort("rays cannot be subtracted, only negated")
  }
  if (dim(e1) == 2) {
