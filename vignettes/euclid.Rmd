...
but you could also created it from a vector of points:
...
The parameter names of each geometry can be accessed with `definition_names()`:
...
parameters in a separate column and create as many rows as the cardinality of
...
standard R numerics, whereas `parameter()` will output something else... read
on!

## A note on exactness

You might have paused at the output of `parameter()`:
...
issues with compounding impressions. Normally this is not an issue in
programming (until it is), but in computational geometry it is. Imagine creating
a plane based on three points in 3D space, only to find that one of the points
doesn't lie on the plane... not fun!
...
standard numerics (e.g. `approx_area()`).
...
between geometries, or... you get the idea. Please consult the thorough
documentation to dive deeper into the possibilities of euclid
