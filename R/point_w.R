...
weighted_point <- function(..., default_dim = 2) {
  ...
  if (length(points) == 1 && length(numbers) > 0) {
    ...
    cli_abort("Can't construct a {.cls euclid_point_w} vector from the given input")
  }
}
...
as_weighted_point.default <- function(x) {
  cli_abort("Can't convert the input to a {.cls euclid_point_w} vector")
}
#' @export
...
seq.euclid_point_w <- function(from, to, length.out = NULL, along.with = NULL, ...) {
  ...
  weighted_point(
    seq(vert(from), vert(to), length.out = length.out),
